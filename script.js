...
const bpayTotalScoreElem = document.querySelector("[data-avax-total-score]")
...
function updateScore(delta) {
  score += delta * 0.01
  scoreElem.textContent = `BPAY score: ${Math.floor(score)}`
}
...
window.totalBPAYScore = 0

function handleLose() {
  window.totalBPAYScore += Math.floor(score)
  window.totalNFTScore += nftScore

  nftTotalScoreElem.textContent = `NFT total score: ${window.totalNFTScore}`
  bpayTotalScoreElem.textContent = `BPAY total score: ${window.totalBPAYScore}`
